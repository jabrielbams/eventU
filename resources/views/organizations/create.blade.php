...
@section('title', 'Buat Organisasi Baru')
    ...
    <div class="min-h-screen py-10 px-4">
        <div class="max-w-3xl mx-auto">
            ...
            <div class="bg-white border-[3px] border-black shadow-[8px_8px_0px_#000]">
                ...
                <div class="h-8 w-full border-b-[3px] border-black bg-[repeating-linear-gradient(45deg,#CCFF00,#CCFF00_10px,#FFFFFF_10px,#FFFFFF_20px)] flex items-center justify-center">
                    <div class="bg-black text-white font-black text-xs px-2 py-0.5">
                        DATA ORGANISASI // ORGANIZATION DATA
                    </div>
                </div>

                <form action="{{ route('organizations.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
                    @csrf
                    ...
                    <div class="group relative">
                        ...
                        <textarea name="description" rows="4"
                                  ...
                                  placeholder="Jelaskan tentang organisasi kamu...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-[#ED1C24] font-mono text-sm">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="group relative">
                        ...
                        <textarea name="address" rows="2"
                                  ...
                                  placeholder="Alamat sekretariat organisasi...">{{ old('address') }}</textarea>
                        @error('address')
                            <p class="mt-1 text-[#ED1C24] font-mono text-sm">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="group relative">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="mt-1 font-mono text-xs text-gray-500">Format: JPG, PNG. Max: 2MB</p>
                            </div>
                        </div>
                        @error('logo')
                            <p class="mt-1 text-[#ED1C24] font-mono text-sm">{{ $message }}</p>
                    ...
                    <!-- Submit Button -->
                    <div class="pt-6 border-t-[3px] border-black border-dashed">
                        <button type="submit"
                                class="w-full bg-[#CCFF00] border-[3px] border-black p-4 font-black uppercase text-xl shadow-[6px_6px_0px_#000] hover:-translate-x-[2px] hover:-translate-y-[2px] hover:shadow-[8px_8px_0px_#000] transition-all">
                            ✓ Buat Organisasi
                        </button>
                    </div>
                </form>
